...
if __name__ == "__main__":
    ...
    parser.add_argument("--corrupted_image_topic", default="/dvs/image_corrupted", help="Corrupted image topic")
    parser.add_argument("--corrupted_every", default=1, help="Extract 1 every 1 corrupted images")
    parser.add_argument("--output_corrupted_image_folder", default="extracted_corrupted", help="Folder where to extract the corrupted images")
    ...
    path_to_corrupted_frames_file = os.path.join(args.output_folder, '{}_corrupted_frames.txt'.format(output_name))

    bridge = CvBridge()
    corrupted_img_msg_sum = 0

    with open(path_to_events_file, 'w') as events_file:
        with open(path_to_corrupted_frames_file, 'w') as corrupted_frames_file:
            with rosbag.Bag(args.bag, 'r') as bag:
                ...
                for topic, msg, t in bag.read_messages():
                    ...
                    if topic == args.corrupted_image_topic:
                        corrupted_idx += 1
                        if (corrupted_idx - 1) % args.corrupted_every != 0:
                            continue
                        save_dir = os.path.join(args.output_corrupted_image_folder, output_name)
                        if not os.path.exists(save_dir):
                            os.makedirs(save_dir)
                        save_name = os.path.join(save_dir, '{}.png'.format(corrupted_img_msg_sum))
                        corrupted_frames_file.write('{} {}\n'.format(corrupted_img_msg_sum, timestamp_str(msg.header.stamp)))
                        corrupted_img_msg_sum += 1
    ...
    if not args.no_zip:
        ...
        # Remove events.txt
        if True:#query_yes_no('Remove text file {}?'.format(path_to_events_file)):
